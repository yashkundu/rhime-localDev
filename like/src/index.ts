...
const start = async () => {
    try {
        ...
        console.log('Reaction service connected to MongoDb ... ');

        await nats.connect({
            servers: process.env.nats_url
        })
        console.log('Reaction service connected to NATS ... ');

        
        app.listen(Number(process.env.APP_PORT), async () => {
            console.log(`Reaction service is listening on port ${process.env.APP_PORT}...`);
