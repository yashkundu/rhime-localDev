...
it('unauthenticated error', async () => {
    await request(app)
    .post(`/api/user/${new ObjectId()}/toggleUser`)
...
it('Bad userId in the url params', async () => {
    ...
    await request(app)
    .post(`/api/user/oih45hhiuph/toggleUser`)
...
it('simple test for follownig and unfollowing a user', async () => {
    ...
    .get(`/api/user/${info.users[0].userId}/profile`)
    .set('Cookie', info.cookies[0])
    .expect(StatusCodes.OK)
    expect(res.body.messiahCount).toEqual(1)

    res = await request(app)
    .get(`/api/user/${info.users[1].userId}/profile`)
    .set('Cookie', info.cookies[0])
    .expect(StatusCodes.OK)
...
it('complex test for following and unfollowing a user', async () => {
    ...
    for(const [x1, x2] of followPairs){
        promises.push(
            request(app)
            .post(`/api/user/${info.users[x2].userId}/toggleUser`)
    ...
    for(let i=0;i<20;i++){
        ...
        .get(`/api/user/${info.users[i].userId}/profile`)
